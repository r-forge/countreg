...
\Abstract{
  This introduction to \proglang{R} functions for count data regression, especially
  the functions \fct{hurdle} and \fct{zeroinfl} from package \pkg{countreg},
  is a somewhat modified version of \cite{countreg:Zeileis+Kleiber+Jackman:2008},
  published in the \emph{Journal of Statistical Software}. Originally, it
  accompanied the package \pkg{pscl} but meanwhile \fct{hurdle}/\fct{zeroinfl}
  have been moved to the \pkg{countreg} package.
  ...
  and \fct{zeroinfl} from the package \pkg{countreg} is introduced. It re-uses
...
\fct{zeroinfl} in the \pkg{countreg} package \citep{countreg:Zeileis+Kleiber:2013},
available from the Comprehensive \proglang{R} Archive Network (CRAN)
at \url{http://CRAN.R-project.org/package=countreg}.
...
functions \fct{hurdle} and \fct{zeroinfl} in package \pkg{countreg} \citep{countreg:Zeileis+Kleiber:2013}.
The original implementation of \cite{countreg:Jackman:2008} was improved
by \cite{countreg:Kleiber+Zeileis:2008} for \pkg{countreg} to make the fitting functions and the
...
\fct{hurdle} function from the \pkg{countreg} package \citep{countreg:Zeileis+Kleiber:2013}.
...
\fct{zeroinfl} function from the \pkg{countreg} package. Both 
...
as the dependent variable and use the health status variables\footnote{In
addition to the variables considered here, \cite{countreg:Zeileis+Kleiber+Jackman:2008}
also employ \code{hosp}, the number of hospital days. As this is more appropriately
used as a dependent variable for medical care rather than a regressor, it is
...
if(file.exists("countreg-models.rda")) {
  ...
  fm_zinb   <- zeroinfl(ofp ~ . | numchron + privins + school + gender, data = dt, dist = "negbin")
  fm_hurdle0<-   hurdle(ofp ~ ., data = dt, dist = "negbin")
  fm_hurdle <-   hurdle(ofp ~ . | numchron + privins + school + gender, data = dt, dist = "negbin")
...
rownames(fm_summary)[1:24] <- ifelse(rownames(fm_summary)[1:24] == "", "", 
  paste("\\code{", rownames(fm_summary)[1:24], "}", sep = ""))
fm_summary[1:7 * 2 - 1,] <- sapply(fm,
  function(x) paste("$", format(round(coef(x)[1:7], digits = 3)), "$\\phantom{)}", sep = ""))
fm_summary[1:7 * 2,] <- sapply(
  c(list("ML-Pois" = vcov(fm_pois), "Adj-Pois" = sandwich(fm_pois)),
  lapply(fm[-(1:2)], function(x) vcov(x))), function(x)
  paste("(", format(round(sqrt(diag(x))[1:7], digits = 3)), ")", sep = ""))
fm_summary[1:5 * 2 + 13,] <- cbind(NA, NA, NA, NA, sapply(fm[5:6], function(x)
  paste("$", format(round(coef(x, model = "zero"), digits = 3)), "$\\phantom{)}", sep = "")))
fm_summary[1:5 * 2 + 14,] <- cbind(NA, NA, NA, NA, sapply(fm[5:6],
  function(x) paste("(", format(round(sqrt(diag(vcov(x)))[-(1:7)], digits = 3)),  ")", sep = "")))
fm_summary[25,] <- sapply(fm, function(x) attr(logLik(x), "df"))
fm_summary[26,] <- paste("$", format(sapply(fm, function(x) round(logLik(x), digits = 1))), "$", sep = "")
fm_summary[27,] <- format(round(sapply(fm, AIC), digits = 1))
fm_summary[28,] <- format(round(sapply(fm, AIC, k = log(nrow(dt))), digits = 1))
fm_summary[29,] <- round(c("ML-Pois" = sum(dpois(0, fitted(fm_pois))),
  "Adj-Pois" = NA,
  "Quasi-Pois" = NA,
...
fm_summary[c(14, 24, 29)] <- paste(fm_summary[c(14, 24, 29)], "\\hline")
...
likewise their \proglang{R} implementation in package \pkg{countreg} re-uses
...
paper. Here, however, we use the full model to show that the tools in \pkg{countreg}
